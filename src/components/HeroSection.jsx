...
export default function HeroSection() {
  return (
    <section id="inicio" className="relative min-h-screen pt-28 pb-16 lg:pt-36 lg:pb-24 overflow-hidden flex items-center w-full">
      ...
      />
      <div className="absolute inset-0 bg-gradient-to-t from-dark-950 via-dark-950/85 to-dark-950/40 -z-10" />
      ...
      <div className="w-full px-4 sm:px-8 lg:px-12 xl:px-16 2xl:px-20">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center">
          ...
          <div className="lg:col-span-7 flex flex-col items-start text-left">
            {/* Top Badge */}
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-gold/10 border border-gold/30 text-gold-light text-xs font-semibold uppercase tracking-widest mb-6 backdrop-blur-sm animate-pulse-slow">
              <Sparkles className="w-3.5 h-3.5 text-gold" />
              <span>Agências Araújo • Fotografia & Eventos RJ</span>
            </div>

            {/* Official Tagline */}
            <p className="font-script text-3xl md:text-4xl text-gold-300 mb-2 drop-shadow-sm tracking-wide">
              "{CONTACT_INFO.tagline}"
            </p>

            {/* Main Headline */}
            <h1 className="font-display text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tight text-white leading-[1.12] mb-6">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 w-full sm:w-auto mb-10">
              ...
              <a
                ...
                className="inline-flex items-center justify-center gap-3 px-7 py-4 text-sm font-semibold text-slate-200 hover:text-white bg-slate-900/90 hover:bg-slate-800/90 border border-slate-700/80 hover:border-gold-500/50 rounded-xl backdrop-blur-sm transition-all duration-300 hover:scale-[1.02]"
              >
            ...
            <div className="grid grid-cols-3 gap-6 pt-6 border-t border-slate-800/80 w-full max-w-xl">
              <div>
                ...
                <div className="text-xs text-slate-400 mt-0.5">Momentos Eternizados</div>
              </div>
              <div>
                <div className="text-2xl font-bold font-display text-gold">100%</div>
                <div className="text-xs text-slate-400 mt-0.5">Clientes Encantados</div>
              </div>
              <div>
                <div className="text-2xl font-bold font-display text-white">Rio de Janeiro</div>
                <div className="text-xs text-slate-400 mt-0.5">& Região Metropolitana</div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-5 relative w-full flex justify-center lg:justify-end">
            ...
            <div className="relative w-full max-w-md">
              ...
              <div className="hidden sm:flex absolute -top-4 -right-4 bg-dark-900/90 backdrop-blur-md px-3.5 py-2 rounded-xl border border-white/10 shadow-xl items-center gap-2">
                <Award className="w-4 h-4 text-gold" />
                <span className="text-xs font-medium text-slate-200">Excelência Técnica & Sensibilidade</span>
              </div>
        ...
        <div className="mt-16 pt-8 border-t border-slate-800/80">
          <div className="text-center text-xs uppercase tracking-widest text-slate-400 font-semibold mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {CONTACT_INFO.services.map((srv) => (
              <a
                key={srv.id}
                href="#servicos"
                className="group p-4 rounded-xl bg-dark-900/60 hover:bg-dark-850/80 border border-slate-800 hover:border-gold-500/40 transition-all duration-300 flex items-center gap-3.5"
              >
                <div className="w-10 h-10 rounded-lg bg-gold/10 group-hover:bg-gold/20 flex items-center justify-center text-gold transition-colors shrink-0">
                  <Camera className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="text-sm font-bold text-white group-hover:text-gold transition-colors tracking-wide">
                    {srv.title.toUpperCase()}
                  </h3>
                  <p className="text-xs text-slate-400 line-clamp-1">{srv.subtitle}</p>
                </div>
              </a>
            ))}
          </div>
